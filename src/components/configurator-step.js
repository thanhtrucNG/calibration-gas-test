...
export function createConfiguratorStep({ field, title, number, options, selected, visual, onSelect, uiState = {} }) {
  ...
  function renderOptions() {
    ...
    status.textContent = matches.length || selected ? '' : t('No matching mixtures. Try another gas or code.');
  }
  ...
  if (options.length > 8) {
    const label = element('label', 'concept-filter-label', t('Filter these mixtures'));
    const input = element('input', 'concept-filter');
    input.type = 'search';
    input.id = `filter-${field}`;
    input.placeholder = t('Type a gas or product code');
...
export function describeOption(field, option, { mapping, families, catalogue }) {
  ...
  if (field === 'config_concept') {
    ...
    detail = `${t('Product code')} ${product.barcode} · ${product.dimensions_display}`;
